...
class UserData:
    ...
    #     key_data_usa = self.get_keys(user_id, 6) if self.get_keys(user_id, 6) else None
    #
    #     lst = []
    #     lst.extend([key_data_amster, key_data_german, key_data_kz, key_data_russia, key_data_turkey, key_data_usa])
    #     return lst

    def get_user_keys_info(self, user_id):
        try:
            sql = ''' SELECT      u.name,     o.server_id,     o.key_value,     u.stop_date FROM     outline_keys o LEFT JOIN user_keys u ON     o.key_id = u.key_id WHERE     u.user_id = %s'''
    ...
    def get_key_id_with_key_name_and_user_id(self, key_name, user_id):
        try:
            query = """SELECT key_id FROM user_keys WHERE name = %s and user_id = %s"""

            result = self.execute_query(query, (key_name, user_id,))

            return result[0][0]
        except Exception as e:
            logger.error(f"QUERY_ERROR - get_key_id_with_key_name_and_user_id - {e}")
    ...
    def get_key_name(self, key_id):
        # запрос для получени ключа, имени ключа, даты окончания сервер 1
        query = """SELECT name from user_keys WHERE key_id = %s"""
        try:
            result = self.execute_query(query, (key_id,))
            return result[0]
        except Exception as e:
            logger.error(f"QUERY_ERROR - get_key_name - {e}")
